...
def selection_of_triples_with_argmax(y_hat: torch.tensor):
    ...
    nonzeros_item_ids, nonzeros_rel_ids = all_of_srt_are_present.nonzero(as_tuple=True)   # 分别记录了横纵坐标   判断每一个detection属于哪个类别？
...
def prediction2triples(prediction, texts, offsets_mapping, tokenized):
    ...
    with torch.no_grad():
        ...
        nonzeros_item_ids, nonzeros_rel_ids = selection_of_triples_with_argmax(y_hat)     # nonzeros_item_ids一维的list，记录了所有可能的结果如113
...
def decode(outputs, entities, length):
    ...
    for instance, ent_set in zip(outputs, entities):
        ent_set = set(ent_set)
        predicts = []
        ...
        for cur in reversed(range(l)):
            ...
            for pre in range(cur+1):
                ...
                if instance[cur, pre] > 1: 
                    ...
                    heads.append(pre)
                # NNW
                if pre < cur and instance[pre, cur] == 4:  # 我把NNW变成4了，之前是1
                    # cur node
                    for head in heads:
...
def decode_prediction(outputs, length):
    '''
    直接解码出结果，不计算指标
    '''
    class Node:
    ...
    for batch_outputs in outputs:
        for instance in batch_outputs:
            ...
            predicts = [convert_index_to_text(x[0], x[1]) for x in predicts]
            decode_entities.append([convert_text_to_index(x) for x in predicts])
    outputs_all.append(decode_entities)
    return outputs_all