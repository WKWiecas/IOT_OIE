...
import math
import torch
from OT.my_utils import spherical_kmeans
import torch.nn.functional as F

def \
        sinkhorn(dot, mask=None, eps=1e-03, return_kernel=False, max_iter=100):
    """
...
def multihead_attn(input, target, mask=None, eps=1.0, return_kernel=False,
                   max_iter=100, log_domain=False, position_filter=None, iou_cost=None):
    ...
    # K = torch.tensordot(input, target, dims=[[-1], [-1]]) #K: n,in_size,m,out_size
    
    # 用-iou作为优化目标   
    # iou cost [num_filtered_detections, 14]
    K = (-1) * iou_cost.unsqueeze(0).repeat(n,1,1)   # 每个batch用的同一套iou
    K = K.unsqueeze(-2)   #[n, num_filtered_detectionsm, 1, 14]

    K = K.permute(0, 2, 1, 3)#K: n, m, in_size, out_size
